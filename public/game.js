...
class playGame extends Phaser.Scene {
    ...
    create(){
        ...
        this.debug.captionStyle = {
            fill: '#7fdbff',
            fontFamily: 'monospace',
            lineSpacing: 4
        };

        this.debug.captionTextFormat = (
            'Total:     %1\n' +
            'Player X:  %2\n' +
            'Player Y:  %3\n' + 
            'Distance:  %4\n' +
        ...
        this.debug.graphics = this.add.graphics();
    ...
    update(){
        ...
        let nextStart = 5000;
        if (this.player.nextPlatform) {
            nextStart = this.player.nextPlatform.getBounds().left;
        }
        this.platformGroup.children.iterate((platform) => {
            const platStart = platform.getBounds().left;
            if ((platStart > this.player.getBounds().right && platStart < nextStart)){
